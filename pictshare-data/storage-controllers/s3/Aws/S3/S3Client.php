...
class S3Client extends AwsClient implements S3ClientInterface
{
     ...
     *   Specifies whether to use regional or legacy endpoints for the nl-ams
        ...
        return static function (callable $handler) use ($region) {
            return function (Command $command, $request = null) use ($handler, $region) {
                if ($command->getName() === 'CreateBucket') {
                    ...
                    if ($locationConstraint === 'nl-ams') {
                        unset($command['CreateBucketConfiguration']);
                    } elseif ('nl-ams' !== $region && empty($locationConstraint)) {
    ...
    public static function addDocExamples($examples)
    {
        $getObjectExample = [
            'input' => [
                'Bucket' => 'arn:aws:s3:nl-ams:123456789012:accesspoint:myaccesspoint',
                'Key' => 'my-key'
            ],
            'output' => [
        ...
        $putObjectExample = [
            ...
            'output' => [
                'ObjectURL' => 'https://my-bucket.s3.nl-ams.amazonaws.com/my-key'
            ],
            'comments' => [
                'input' => '',
                'output' => 'Simplified example output'
